(function() {
    ...
    function Element(elementClass) {
        ...
        //this.setup();
        this.addedSetupListener=this.addEventListener("added",function(e){
            e.target.AddedSetup();
        });
    }
    ...
    element.AddedSetup = function() {
        //this.SizableSetup();
        this.removeEventListener("added",this.addedSetupListener);
    ...
    element.hitTest=function(el) {
        if (el && el != this && el.parent) {
            if (( (!this.getElement()._hitArea && this.hitTestObject(el) && !( this.hitTestContents(el)) && this.isInsideMe(el) ) 
            )) {
                return true;
            }else if (this.getElement()._hitArea && el.getElement() && el.elementType=="middleevent") {
				return this.getElement().hitAreaHitTest(el.getElement());
                //return HitTest.complexHitTestObject(this.getElement()._hitArea, el.getElement(),10);
            }
        }
        return false;
    }
    ...
    element.isInsideMe=function(el) {
        if (!this.getElement()._hitArea) {
            ...
            var thisX = (this.x+this._sizableElement.x) - (this._width / 2);
            var thisY = (this.y+this._sizableElement.y) - (this._height / 2);
            ...
            if ((this._width > el.getRealWidth() && this._height > el.getRealHeight()) &&
            ((el.getRealWidth() + elX) < (this._width + thisX) && (el.getRealHeight() + elY) < (this._height + thisY)) &&
            ( elX>thisX && elY>thisY )
            ) {
                return true;
            }
    ...
    element.onResizeDrop=function(e) {
        ...
        this.dispatchEvent(new facilis.Event(facilis.Drag.STOP_EVENT));
	...
	element.updateContents=function(){
		if(this.contents && this.contents.length>0)
			facilis.MultiDrag.getInstance().drag(this,this.contents);
	}
    ...
    element.addContent=function(el) {
        if(this.contents.length==0)
			this.addEventListener(facilis.AbstractElement.ELEMENT_MOVED, this.updateContents.bind(this));
    ...
    element.removeContent=function(el) {
		...
		if(this.contents.length==0)
			this.removeEventListener(facilis.AbstractElement.ELEMENT_MOVED, this.updateContents);
    }
